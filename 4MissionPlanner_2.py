...
class Drone:
    ...
    async def run(self):
        ...
        # Predefined list of mission points
        mission_points = [
        	[47.3979621, 8.5456644, 0],  # north ~5m
    		[47.3979621, 8.5457100, 0],  # east ~5m
    		[47.3979171, 8.5457100, 0],  # south ~5m
    		[47.3979171, 8.5456644, 2]   # return & land
        ]
        ...
        for point in mission_points:
            try:
                lat, lon, action = point
                coord_list.append((lat, lon, MissionItem.VehicleAction(int(action))))
            except Exception as e:
                print(f"[Input Error] Skipping invalid entry {point}: {e}")
